...
def agent(obs):
    ...
    # Check COW and MILK presence  
    cow_tile = tile_at(farm, COW_TILE)

    cow_is_present = (
        isinstance(cow_tile, dict)
        and cow_tile.get("kind") == "PASTURE"
        and cow_tile.get("animal") == "COW"
    )
    ...
    cow_purchase_planned = False

    if not cow_is_owned and money_available >= COW_COST:
        market_orders.append(["BUY_ANIMAL", "COW", 1])
        money_available -= COW_COST
        cow_purchase_planned = True
    
    # Buy wheat if we own COWs
    if cow_is_owned or cow_purchase_planned:
        wheat_feed_stock = (
            shed_counts["WHEAT"]
    ...
    # If COW is not present, or is present but:
    # - not fed yet, or not cared yet, or there is MILK
    cow_requires_attention = (
        not cow_is_present
        or milk_in_farmer_inventory > 0
        or (
            cow_is_present
            and (
                not cow_tile.get("fed_today", False)
                or not cow_tile.get("cared_today", False)
                or cow_tile.get("yield_units", 0)
                    >= MILK_HARVEST_THRESHOLD
            )
        )
    )

    if cow_requires_attention:
        if pos_current != COW_TILE:
            farmer_action = move_to(pos_current, COW_TILE)

        elif cow_tile is None:
            farmer_action = ["BUILD_PASTURE"]

        elif cow_tile.get("kind") != "PASTURE":
            farmer_action = ["DIG"]
            
        elif cow_tile.get("animal") is None:
        ...
        elif not cow_tile.get("fed_today", False):
            if wheat_in_farmer_inventory > 0:
                farmer_action = ["FEED"]
            elif shed_counts["WHEAT"] > 0:
                farmer_action = ["PICKUP", "WHEAT", 1]
            else:
                farmer_action = ["PASS"]

        elif not cow_tile.get("cared_today", False):
            farmer_action = ["CARE"]

        elif milk_in_farmer_inventory > 0:
            farmer_action = [
                "PLACE",
                "MILK",
            ...
            market_orders.append(
                ["SELL", "MILK", milk_in_farmer_inventory]
            )

        elif (
            cow_tile.get("yield_units", 0)
            >= MILK_HARVEST_THRESHOLD
        ):
            farmer_action = ["HARVEST"]
