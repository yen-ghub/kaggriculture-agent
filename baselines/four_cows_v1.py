...
COW_COST = 400
MILK_HARVEST_THRESHOLD = 1

# Farm hands
HANDS_TO_HIRE        = 5
DAILY_HAND_HIRE_COST = 12
...
HAND_WORK_TILES_EACH = [
    FIRST_QUADRANT_CROP_TILES[:6],
    FIRST_QUADRANT_CROP_TILES[6:12],
...
def agent(obs):
    ...
    #
    active_cow_tiles = list(INITIAL_COW_TILES)

    if SECOND_QUADRANT_NAME in farm["unlocked_quadrants"]:
        active_cow_tiles.extend(
            EXPANSION_COW_TILES[:EXPANSION_COW_COUNT]
        )

    cow_count_target = len(active_cow_tiles)
    cow_feed_reserve = 2 * cow_count_target
    ...
    def choose_hand_action(
        ...
        for position in assigned_tiles:
            # Cow tiles are reserved for the farmer, skip this tile
            if position in COW_TILES:
                continue
            
            tile = tile_at(farm, position)

            # If tile is empty, then plant
            if tile is None:
                can_plant = (
    ...
    ## Get COW and MILK presence (list as dict)
    cow_tiles = {
        position: tile_at(farm, position)
        for position in active_cow_tiles
    }
    
    cow_positions = [
        position
        for position, tile in cow_tiles.items()
        if (
    ...
    cow_in_farmer_inventory = farmer_inventory.get("COW", 0)
    cow_in_any_inventory = sum(
        inventory.get("COW", 0)
    ...
    cows_owned = (
        len(cow_positions)
        + cow_in_shed
        + cow_in_any_inventory
    )

    wheat_in_farmer_inventory = farmer_inventory.get("WHEAT", 0)
    milk_in_farmer_inventory = farmer_inventory.get("MILK", 0)
    milk_in_shed = shed.get("MILK", 0)
    
    
    ## 3.1 Pick a crop based on the current strategy (market price + opponent's crops)
    ...
    ## 3.2 Sell crop of there is any in the shed (loop for each crop)
    for crop in CROPS_MANAGED:
        quantity_to_sell = shed_counts[crop]

        # Reserve some wheat for the COWs
        if crop == "WHEAT":
            quantity_to_sell = max(0,quantity_to_sell - cow_feed_reserve)

        if quantity_to_sell > 0:
            market_orders.append(
                ["SELL", crop, quantity_to_sell]
            )
    ...
    # 3.4 Buy COW if it is below target
    cows_to_buy = max(0, cow_count_target - cows_owned)
    cow_purchase_planned = cows_to_buy > 0

    cow_purchase_cost = cows_to_buy * COW_COST

    if (cows_to_buy > 0 and (money_available >= cow_purchase_cost)):
        market_orders.append(["BUY_ANIMAL", "COW", cows_to_buy])
        money_available -= cow_purchase_cost
    
    # Buy wheat if we own COWs
    if cows_owned or cow_purchase_planned:
        wheat_feed_stock = (
            shed_counts["WHEAT"]
            + wheat_in_farmer_inventory
        )
        wheat_to_buy = max(0, cow_feed_reserve - wheat_feed_stock)
    ...
    # COW is a big investment so it takes priority if it requires attention (also it is at the starting tile)
    def get_wheat_action():
        if pos_current != SHED_ACCESS_TILE:
            return move_to(pos_current, SHED_ACCESS_TILE)

        unfed_cow_count = sum(
            not cow_tiles[position].get("fed_today", False)
            for position in cow_positions
        )

        quantity_to_pickup = min(
            shed_counts["WHEAT"],
            max(1, unfed_cow_count),
        )
    ...
    def choose_cow_action():
        # Deposit harvested Milk before doing more work.
        if milk_in_farmer_inventory > 0:
            if pos_current != SHED_ACCESS_TILE:
                return move_to(pos_current, SHED_ACCESS_TILE)
            ...
            return [
                "PLACE",
                "MILK",
                milk_in_farmer_inventory,
            ]

        # Build pastures and place missing cows.
        setup_targets = [
            position
            for position in active_cow_tiles
            if position not in cow_positions
        ]

        if setup_targets:
            if (cow_in_farmer_inventory == 0 and cow_in_shed > 0):
                if pos_current != SHED_ACCESS_TILE:
                    return move_to(pos_current, SHED_ACCESS_TILE)
                ...
                return [
                    "PICKUP",
                    "COW",
                    min(cow_in_shed, len(setup_targets)),
                ]
            ...
            if target_tile.get("animal") is None:
                if cow_in_farmer_inventory > 0:
                    return ["PLACE", "COW", 1]

                return ["PASS"]

        # Service the cow at the current position first.
        current_cow = cow_tiles.get(pos_current)

        # Action logic block: FEED, CARE or HARVEST
        if (isinstance(current_cow, dict)
                and current_cow.get("kind") == "PASTURE"
                and current_cow.get("animal") == "COW"):
            if not current_cow.get("fed_today", False):
                if wheat_in_farmer_inventory > 0:
                    return ["FEED"]

                return get_wheat_action()

            if not current_cow.get("cared_today", False):
                return ["CARE"]

            if (
                current_cow.get("yield_units", 0)
                >= MILK_HARVEST_THRESHOLD
            ):
                return ["HARVEST"]

        # Travel to another cow that requires attention.
        attention_targets = [
            position
            for position in cow_positions
            if (
                not cow_tiles[position].get("fed_today", False)
                or not cow_tiles[position].get("cared_today", False)
                or cow_tiles[position].get("yield_units", 0)
                    >= MILK_HARVEST_THRESHOLD
            )
        ]
        ...
        if attention_targets:
            ...
            target_cow = cow_tiles[target]

            if (
                not target_cow.get("fed_today", False)
    ...
    cow_action = choose_cow_action()

    if cow_action is not None:
        farmer_action = cow_action

        if (farmer_action[0] == "PLACE" and farmer_action[1] == "MILK"):
            market_orders.append([
                "SELL",
                "MILK",
                milk_in_farmer_inventory,
