...
COW_COUNT = len(COW_TILES)
COW_COST = 400
COW_FEED_RESERVE = 2 * COW_COUNT
...
HAND_WORK_TILES_EACH = [
    TILE_ROUTE[:7],              # Existing first-hand zone
    TILE_ROUTE[7:13],             # Existing second-hand zone
...
def agent(obs):
    ...
    # Check COW and MILK presence (list as dict)
    cow_tiles = {
    ...
    for crop in CROPS_MANAGED:
        ...
        if crop == "WHEAT":
            quantity_to_sell = max(
                0,
                quantity_to_sell - COW_FEED_RESERVE,
            )
    ...
    # Buy COW if there is none
    cows_to_buy = max(0, COW_COUNT - cows_owned)
    ...
    ####################################
    # 7. Closing market order (buy seeds)
